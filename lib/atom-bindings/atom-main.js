...
module.exports = function({dispatch, Lupa, analysis}) {
    ...
    return {
        ...
        activate(state) {
            let onDidStopChanging;
            function updateEditor(editor) {
                if (onDidStopChanging) {
                    onDidStopChanging.dispose();
                    onDidStopChanging = null;
                }
                if (editor && editor.buffer) {

                    onDidStopChanging = editor.onDidStopChanging(change => {
                        dispatch({
                            type: 'setActiveFile',
                            file: {
            ...
            const panelPosition = atom.config.get('atom-lupa.positionOfPanel');
            if (panelPosition == 'right') {
                sidepanel = atom.workspace.addRightPanel({item: el});
            } else {
            ...
            window.lupaShowWarningsAboutLongFunctions = atom.config.get('atom-lupa.shouldShowWarningsAboutLongFunctions');
