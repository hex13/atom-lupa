...
export function EntityName({name, entity, icons, style}) {
    ...
    //TODO
        ...
        //
        return <span className="source js"><span className="lupa-entity-name entity name function">
            {icons? <span>{icons} </span> : ''}
            <span
                style={style}
                title={entity.file && entity.file.path}
            >{finalName}</span></span></span>;
}
...
export class Structure extends React.Component {
    ...
    render() {
        ...
        if (this.state.phrase.indexOf('..') === 0) {
            if (!window.lupaEntities) {
                window.lupaEntities
            }
            metadata = (window.lupaEntities || []);
