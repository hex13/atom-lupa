    ...
    render() {
        ...
        return <span>
            ...
            <em style={{color: '#666'}}> {superClass.name? '(' + superClass.name + ')' : ''}</em>
            {
                ...
                <ul>
                {
                    entity.functions && entity.functions.map(meth => <li><Function entity={meth} /></li>)
                }
                </ul>
            }
        </span>;
...
export class Structure extends React.Component {
    ...
    renderEntity(entity, i, arr) {
        const onClick = this.props.onClickEntity;
        const prev = arr[i - 1];
        const shouldShowType = !prev || prev.type != entity.type;
        //window.lupaActiveEntty
            return <div
            className="lupa-entity"
            onClick={onClick && onClick.bind(null, entity)}
            onFocus={() => this.changeActiveEntity(entity)}
            onBlur={() => this.changeActiveEntity(null)}
            tabIndex="0"
        >
