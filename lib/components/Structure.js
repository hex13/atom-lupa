...
function Function({entity, short}) {
    ...
    if (entity.isMethod) {
        icons.push(<span className="lupa-badge lupa-badge-method">M</span>)
    }
...
export function EntityName({name, entity, icons}) {
    ...
    render() {
        ...
        function filterByPhrase(item) {
            ...
            if (indexOfColon != -1) {
                ...
                const propValue = phrase.substr(indexOfColon + 1);
                return item.hasOwnProperty(propName)?
                    (item[propName] + '').indexOf(propValue) != -1
                    : false;
            }
            if (item.name) {
