...
export function Function({entity, short}) {
    ...
    return <div style={{display:'inline-block'}}>
        ...
        {
            entity.argumentOf.name && <em style={parentFuncStyle}>{entity.argumentOf.name.split('.').pop()}(</em>
        }
        <EntityName entity={entity} name={name} icons={icons}/>
        { entity.argumentOf.name && <span style={parentFuncStyle}>)</span> }
    </div>
}
...
export class Structure extends React.Component {
    ...
    render() {
        ...
        const finalMetadata = this.state.filter(this.props.metadata, this.props.allMetadata);
        ...
        >
            ...
            <div className="native-key-bindings">
                ...
                />
                {this.props.showInfo && <div>
                    tab/shift tab -> next/prev<br/>
                    enter -> edit<br />
                    esc -> cancel<br/><br />
                    You can begin search with two dots to look in all indexed files. E.g. <br/><em>..type:class</em>
                    <br />
                    You can also can resize panel by dragging on border.
                    <br/>
                    made by <a href="https://twitter.com/hex13code" className="hex13">⬡hex13</a>
                </div>}
            </div>
            <div>
            <EntityList
