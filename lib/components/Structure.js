...
export function JsonItem({entity, short}) {
    const color = ['#aaa', '#888'][entity.level % 2];
    return <span style={{color}}>
        <EntityName
            entity={entity}
            name={short? entity.basename : entity.name}
        />
    </span>
}
...
export function Function({entity, short}) {
    ...
    return <div style={{display:'inline-block'}}>
        {
            window.lupaShowWarningsAboutLongFunctions && !short && isLong &&
...
export class Structure extends React.Component {
    ...
    render() {
        ...
        const finalMetadata = sort(metadataFilteredBySearchBox
            .filter(item => ignoredTypes.indexOf(item.type) == -1));
