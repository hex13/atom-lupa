...
export class Lupa extends React.Component {
    ...
    componentDidMount() {
        ...
        this.navigationActions = NavigationActions(dispatch);
        ...
        setInterval(() => {
            ...
            if (pos) {
                ...
                const el = <StatusBar
                    ...
                    onMouseOut={
                        () => navigationActions.RemovePreview()
                    }
                    onMouseOver={
                        (entity) => {
                            navigationActions.SetPreview(entity);
                        }
                    }
                    onClick={(entity, e)=> {
                        // TODO this doesn't work because of mouseOver and mouseLeave events
                        // at least It didn't work back then. But now it seems to work. Weird.
                        navigationActions.GoTo(entity, false, {select: e.altKey});
                    }}
                    entities={entities}/>;
    ...
    getStructureEvents(dispatch, metadata) {
        ...
        return {
            ...
            onClick(e) {
                const el = e.target.closest('.lupa-entity');
                if (el) {
                    navigationActions.GoTo(el, false, {select: e.altKey});
                }
            }
        }
    }
