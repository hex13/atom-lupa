...
const DEFAULT_CONFIG = {
    comment: 'This is only default config! It may not be appropriate for your project. For more config, see atom.io/packages/atom-lupa',
    filePattern: "src/**/*.js",
}
    ...
    return (next) => (action) => {
        if (action.type == 'indexProject') {
            ...
            if (activeFile) {
                ...
                alert("You have to open file first and then click Index Project.");
                next({type: 'indexed', result: 0});
            }
        }
